...
interface Props {
  onResult: (tasks: DailyTask[], date: string, transcription: string) => void
}

type State = 'idle' | 'recording' | 'processing' | 'error'

export default function MicButtonTasks({ onResult }: Props) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      for (const mime of mimeTypes) {
        if (MediaRecorder.isTypeSupported(mime)) { selectedMimeType = mime; break }
      }
      ...
      mr.onstop = async () => {
        ...
        try {
          const res = await fetch('/api/voice-tasks', { method: 'POST', body: fd })
          const data = await res.json()
          if (!res.ok) throw new Error(data.error ?? 'Erro ao processar')
          onResult(data.tasks, data.date, data.transcription)
          setState('idle')
        } catch (e: any) {
    ...
    <div className="flex flex-col items-center gap-3">
      <button
        ...
        className={`w-20 h-20 rounded-full flex items-center justify-center shadow-2xl transition-all select-none touch-none ${
          state === 'recording' ? 'bg-red-500 scale-110 shadow-red-500/40' :
          state === 'processing' ? 'bg-zinc-700 cursor-not-allowed' :
          state === 'error' ? 'bg-red-900 hover:bg-red-800' :
      ...
      {state === 'error' && errorMsg && <p className="text-xs text-red-400 text-center max-w-xs">{errorMsg}</p>}
    </div>
  )
}
